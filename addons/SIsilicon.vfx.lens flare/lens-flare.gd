...
func _init():
	...
	screen.mesh = mesh
	# Adjust the scale carefully considering your shader's requirements
	screen.scale = Vector3(1,1,1) * pow(2.0,30); # Be cautious with very large scales
	add_child(screen)
	
	# Assign the material early to ensure it's ready when needed
	material = preload("lens-flare-shader.tres").duplicate()
