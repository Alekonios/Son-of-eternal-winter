...
func release_on_exit(p_base: Node, p_instance: Node, p_finish_playing: bool = false) -> void:
	...
	if not p_base.tree_exiting.is_connected(p_instance.release):
...
func _instance_manual(p_bank_label: String, p_event_name: String, p_reserved: bool = false, p_bus: String = "", p_poly: bool = false, p_attachment = null, p_autorelease: bool = true) -> Variant:
	if not has_loaded:
	...
	if p_autorelease:
		release_on_exit(p_attachment, player, false)
